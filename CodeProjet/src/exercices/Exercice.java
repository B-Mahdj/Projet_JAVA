...
import application.Montage;
import film.Film;

public class Exercice {
	/*
	 * Dans chacune des 5 méthodes suivantes, une instruction doit être modifiée.
	 * Elles correspondent aux 5 opérations élémentaires du sujet.
	 * 
	 * Vous pouvez compléter les méthodes dans l'ordre que vous voulez. Vous pouvez
	 * exécuter les tests même si vous n'avez pas encore complété toutes les
	 * méthodes.
	 * 
	 */
	public static Film collage(Film f, Film g) {
		...
		Film c = Montage.Collage(f, g); // à modifier
	...
	public static Film extrait(Film g, int d, int f) {
		...
		// celle de g).
		Film e = Montage.ExtraitFilm(g, d, f); // à modifier
	...
	public static Film cadre(Film f) {
		...
		// des caractères '*'.
		Film c = null; // à modifier
		c = Montage.Encadrement(f);
	...
	public static Film incrustation(Film f, Film g, int li, int co) {
		...
		// ignorées.
		// - Si f est composé de plus d'images que g, les dernières images de f
		...
		Film i = Montage.Incrustation(f, g, li, co); // à modifier
